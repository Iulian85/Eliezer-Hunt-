...
import * as THREE from 'this-is-not-needed-but-imported-as-three';
...
const DriftingCoin = ({ coinRef, initialPos, onDistanceChange, onEscape, isPaused }: { coinRef: React.MutableRefObject<THREE_LIB.Group | null>, initialPos: THREE_LIB.Vector3, onDistanceChange: (d: number) => void, onEscape: () => void, isPaused: boolean }) => {
    const { camera } = useThree();
    const velocity = useMemo(() => {
        const angle = Math.random() * Math.PI * 2; 
        const speed = 0.6 + Math.random() * 0.5; 
        return new THREE_LIB.Vector3(Math.cos(angle) * speed, 0, Math.sin(angle) * speed);
    }, [initialPos]);
...
export const ARView: React.FC<ARViewProps> = ({ target, onClose, onCollect }) => {
    ...
    const coinGroupRef = useRef<THREE_LIB.Group>(null);
    const [permissionError, setPermissionError] = useState(false);
    const [cameraActive, setCameraActive] = useState(false);
    
    const [coinPos, setCoinPos] = useState(() => new THREE_LIB.Vector3((Math.random() - 0.5) * 4, 1.5, -6));
    ...
    useEffect(() => {
        if (targetId) {
            ...
            setCoinPos(new THREE_LIB.Vector3((Math.random() - 0.5) * 5, 1.2, -7));
            ...
            try {
                ...
                if (mounted && videoRef.current) {
                    ...
                    videoRef.current.onloadedmetadata = () => {
                        videoRef.current?.play().then(() => { if (mounted) setCameraActive(true); });
                    };
                }
    ...
    const forceCameraPlay = () => { if (videoRef.current) videoRef.current.play().then(() => setCameraActive(true)); };
    ...
    const handleEscape = () => { 
        if (!hasEscaped && !collecting && !isRespawning && !gbRevealed) { 
            ...
            setTimeout(() => {
                setIsRespawning(true);
                setTimeout(() => {
                    setCoinPos(new THREE_LIB.Vector3((Math.random() - 0.5) * 4, 1.2, -6));
    ...
    return (
        <div className="fixed inset-0 z-[10010] bg-black overflow-hidden flex flex-col font-sans" onClick={forceCameraPlay}>
            <div className="absolute inset-0 z-0">
                {!permissionError ? <video ref={videoRef} className="w-full h-full object-cover" autoPlay playsInline muted /> : (
            ...
            <div className="absolute inset-0 z-20 pointer-events-none flex flex-col justify-between p-6">
                <div className="flex justify-between items-start pointer-events-auto">
                    <div className="bg-black/40 backdrop-blur-xl px-4 py-2 rounded-xl border border-white/10">
                        ...
                        <p className="text-white text-xs font-black flex items-center gap-2"> {cat === 'GIFTBOX' ? <Gift size={14}/> : <Zap size={14}/>} {target?.spawn.name || "Searching..."} </p>
                    </div>
                    ...
                    <div className="fixed inset-0 z-[10030] flex items-center justify-center p-6 bg-black/60 backdrop-blur-sm pointer-events-auto">
                        <div className="bg-slate-950 border-2 border-amber-400 p-8 rounded-[2.5rem] flex flex-col items-center gap-5 animate-in zoom-in duration-500 shadow-2xl">
                            ...
                            <div className="text-center">
                                ...
                                <span className="text-4xl font-black text-white flex items-center gap-2"> {wonTon || wonPoints} {wonTon ? <Wallet size={24}/> : <Coins size={24}/>} </span>
                            </div>
                        </div>
                    </div>
                )}
