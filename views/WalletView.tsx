...
import { processWithdrawTON, logAdStartFirebase } from '../services/firebase';
...
export const WalletView: React.FC<WalletViewProps> = ({ 
    ...
    const handleWatchAd = async () => {
        if (timeRemaining || loadingAd) return;
        setLoadingAd(true);
        
        // Log-am inceputul pe server (Securitate Strat 3)
        if (userState.telegramId) {
            await logAdStartFirebase(userState.telegramId);
        }
        ...
        <div className="h-full w-full p-6 overflow-y-auto pb-40 no-scrollbar bg-slate-950">
            ...
            <div className="mb-8">
                ...
                <div className="glass-panel border border-white/5 rounded-3xl p-6 shadow-xl space-y-4">
                    ...
                    <div className="space-y-4">
                        <div className="flex justify-between items-center"><div className="flex items-center gap-2"><Target size={14} className="text-slate-400" /><span className="text-[11px] text-slate-400 font-medium">Gameplay (Urban/Mall)</span></div><span className="text-xs font-mono font-bold text-white">{gameplayBalance.toLocaleString()}</span></div>
                        <div className="flex justify-between items-center"><div className="flex items-center gap-2"><Crown size={14} className="text-purple-400" /><span className="text-[11px] text-purple-400 font-medium">Rare Coins (Landmarks)</span></div><span className="text-xs font-mono font-bold text-purple-400">{rareBalance.toLocaleString()}</span></div>
                        <div className="flex justify-between items-center"><div className="flex items-center gap-2"><Sparkles size={14} className="text-blue-400" /><span className="text-[11px] text-blue-400 font-medium">Event Coins</span></div><span className="text-xs font-mono font-bold text-blue-400">{eventBalance.toLocaleString()}</span></div>
                        <div className="flex justify-between items-center"><div className="flex items-center gap-2"><Clock size={14} className="text-slate-400" /><span className="text-[11px] text-slate-400 font-medium">Daily Supply (Rewards)</span></div><span className="text-xs font-mono font-bold text-white">{dailySupplyBalance.toLocaleString()}</span></div>
                        <div className="flex justify-between items-center"><div className="flex items-center gap-2"><Megaphone size={14} className="text-red-500" /><span className="text-[11px] text-red-500 font-medium">Merchant Drops</span></div><span className="text-xs font-mono font-bold text-red-500">{merchantBalance.toLocaleString()}</span></div>
                        <div className="flex justify-between items-center"><div className="flex items-center gap-2"><Users size={14} className="text-cyan-400" /><span className="text-[11px] text-cyan-400 font-medium">Referral Bonus (Frens)</span></div><span className="text-xs font-mono font-bold text-cyan-400">{referralBalance.toLocaleString()}</span></div>
                    </div>
                </div>
            </div>
            ...
            <div className="glass-panel p-8 rounded-3xl mb-8 relative overflow-hidden border border-blue-500/20 shadow-[0_0_30px_rgba(37,99,235,0.1)]">
                ...
                <div className="relative z-10">
                    <div className="flex items-center gap-2 mb-6"><div className="p-2 bg-blue-500/20 rounded-lg"><Wallet className="text-blue-400" size={18} /></div><span className="text-[10px] text-blue-200 font-black uppercase tracking-widest">TON Asset Reserve</span></div>
                    <div className="mb-8 text-center"><div className="text-5xl font-black text-white font-mono tracking-tighter">{tonBalance.toFixed(2)} <span className="text-lg text-blue-400">TON</span></div><p className="text-[9px] text-slate-500 uppercase mt-2 font-bold tracking-widest">Verified Multi-Chain Balance</p></div>
                    <button onClick={handleWithdraw} disabled={tonBalance < 10 || withdrawing} className={`w-full py-4 rounded-2xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-2 transition-all active:scale-95 shadow-xl ${tonBalance >= 10 ? 'bg-white text-blue-900 shadow-blue-400/20' : 'bg-slate-800 text-slate-500 cursor-not-allowed'}`}>{withdrawing ? <Loader2 className="animate-spin" size={16}/> : (withdrawSuccess ? "Success! Check Wallet" : (tonBalance >= 10 ? "Withdraw Assets" : "Min 10 TON"))}{!withdrawing && !withdrawSuccess && tonBalance >= 10 && <ArrowUpRight size={16}/>}</button>
                </div>
            </div>
